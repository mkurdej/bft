...
public:
    ...
    void
    rule_conjunctive_q(cl::Buffer& dQconj12, const cl::Buffer& dQ1,
                       const cl::Buffer& dQ2, cl_uint setSize, cl_ulong count);

    void commonality_to_mass_aos(cl::Buffer& dM, const cl::Buffer& dQ,
                                 cl_uint setSize, cl_ulong count);
    void commonality_to_mass_soa(cl::Buffer& dM, const cl::Buffer& dQ,
                                 cl_uint setSize, cl_ulong count);

    void mass_to_commonality_aos(cl::Buffer& dQ, const cl::Buffer& dM,
                                 cl_uint setSize, cl_ulong count);
    void mass_to_commonality_soa(cl::Buffer& dQ, const cl::Buffer& dM,
                                 cl_uint setSize, cl_ulong count);

    void rule_conjunctive_m_aos(cl::Buffer& dMconj12, const cl::Buffer& dM1,
                                const cl::Buffer& dM2, cl_uint setSize,
                                cl_ulong count);
    void rule_conjunctive_m_soa(cl::Buffer& dMconj12, const cl::Buffer& dM1,
                                const cl::Buffer& dM2, cl_uint setSize,
                                cl_ulong count);

    void create_lidar_grid(cl::Buffer& dFree, cl::Buffer& dOccupied,
                           cl::Buffer& dOmega, const cl::Buffer& dScanAngle,
                           const cl::Buffer& dScanRadius, cl_uint pointCount,
                           GridSize1D gridSizeAngle, GridSize1D gridSizeRadius,
                           cl_float massFree, cl_float massOccupied);

private:
    void loadKernels();
    cl::Kernel loadKernel(cl::STRING_CLASS const& sourceDirectory,
                          cl::STRING_CLASS const& filePath,
                          cl::STRING_CLASS const& kernelName,
                          cl::STRING_CLASS const& flags = "");

    void
    commonality_to_mass(cl::Kernel& kernel, cl::Buffer& dM,
                        const cl::Buffer& dQ, cl_uint setSize, cl_ulong count);
    void
    mass_to_commonality(cl::Kernel& kernel, cl::Buffer& dQ,
                        const cl::Buffer& dM, cl_uint setSize, cl_ulong count);
    // void rule_conjunctive_m(cl::Kernel & kernel, cl::Buffer & dMconj12, const
    // cl::Buffer & dM1, const cl::Buffer & dM2, cl_uint setSize, cl_ulong
    // count);
    ...
    , mQueue(queue)
{
    ...
    cl_int errorCode;
    cl_uint maxWorkgroupSize = static_cast<cl_uint>(
        mDevices[0].getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>(&errorCode));
    // FIXME: C4267 cl_uint is defined as 32-bit, return value is 64-bit
    // (::size_t) on 64-bit platforms
...
{
    commonality_to_mass_1d_aos =
        loadKernel("cl/", "cl/commonality.cl", "commonality_to_mass_1d_aos");
    commonality_to_mass_1d_soa =
        loadKernel("cl/", "cl/commonality.cl", "commonality_to_mass_1d_soa");
    mass_to_commonality_1d_aos =
        loadKernel("cl/", "cl/commonality.cl", "mass_to_commonality_1d_aos");
    mass_to_commonality_1d_soa =
        loadKernel("cl/", "cl/commonality.cl", "mass_to_commonality_1d_soa");
    multiply_1d = loadKernel("cl/", "cl/multiply.cl", "multiply_1d");
    create_lidar_grid_2d =
        loadKernel("cl/", "cl/lidar.cl", "create_lidar_grid_2d");
}

cl::Kernel OpenCL::loadKernel(cl::STRING_CLASS const& sourceDirectory,
                              cl::STRING_CLASS const& filePath,
                              cl::STRING_CLASS const& kernelName,
                              cl::STRING_CLASS const& flags)
{
...
void OpenCL::commonality_to_mass(cl::Kernel& kernel, cl::Buffer& dM,
                                 const cl::Buffer& dQ, cl_uint setSize,
                                 cl_ulong count)
{
    using namespace cl;
    ...
    // Calculate mass M
    {
        ...
        mQueue.enqueueNDRangeKernel(kernel, /* offset = */ NullRange, global,
                                    local);
    }
}

void OpenCL::commonality_to_mass_aos(cl::Buffer& dM, const cl::Buffer& dQ,
                                     cl_uint setSize, cl_ulong count)
{
    commonality_to_mass(commonality_to_mass_1d_aos, dM, dQ, setSize, count);
}

void OpenCL::commonality_to_mass_soa(cl::Buffer& dM, const cl::Buffer& dQ,
                                     cl_uint setSize, cl_ulong count)
{
...
void OpenCL::mass_to_commonality(cl::Kernel& kernel, cl::Buffer& dQ,
                                 const cl::Buffer& dM, cl_uint setSize,
                                 cl_ulong count)
{
    using namespace cl;
...
void OpenCL::mass_to_commonality_aos(cl::Buffer& dQ, const cl::Buffer& dM,
                                     cl_uint setSize, cl_ulong count)
{
    mass_to_commonality(mass_to_commonality_1d_aos, dQ, dM, setSize, count);
}

void OpenCL::mass_to_commonality_soa(cl::Buffer& dQ, const cl::Buffer& dM,
                                     cl_uint setSize, cl_ulong count)
{
...
void OpenCL::rule_conjunctive_m_aos(cl::Buffer& dMconj12, const cl::Buffer& dM1,
                                    const cl::Buffer& dM2, cl_uint setSize,
                                    cl_ulong count)
{
    using namespace cl;
...
void OpenCL::rule_conjunctive_m_soa(cl::Buffer& dMconj12, const cl::Buffer& dM1,
                                    const cl::Buffer& dM2, cl_uint setSize,
                                    cl_ulong count)
{
    using namespace cl;
...
void OpenCL::rule_conjunctive_q(cl::Buffer& dQconj12, const cl::Buffer& dQ1,
                                const cl::Buffer& dQ2, cl_uint setSize,
                                cl_ulong count)
{
    using namespace cl;
    ...
    // Calculate commonality Qconj12
    {
        ...
        mQueue.enqueueNDRangeKernel(multiply_1d, /* offset = */ NullRange,
                                    global, local);
    }
}
...
void
OpenCL::create_lidar_grid(cl::Buffer& dFree, cl::Buffer& dOccupied,
                          cl::Buffer& dOmega, const cl::Buffer& dScanAngle,
                          const cl::Buffer& dScanRadius,
                          const cl_uint pointCount,
                          const struct GridSize1D gridSizeAngle,
                          const struct GridSize1D gridSizeRadius,
                          const cl_float massFree, const cl_float massOccupied)
{
    using namespace cl;

    NDRange global(getRoundedGlobalRange(pointCount, mWorkgroupSize),
                   getRoundedGlobalRange(gridSizeRadius.size, mWorkgroupSize));
...

    {
        ...
        mQueue.enqueueNDRangeKernel(create_lidar_grid_2d,
                                    /* offset = */ NullRange, global, local);
    }
}
