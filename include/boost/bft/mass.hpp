...
public:
    ...
    /// It creates a mass function will all mass attributed to the Conflict set
    /// (Emptyset).
    ...
    /// It creates a mass function will all mass attributed to the Unknown set
    /// (Omega).
    ...
    static mass<FOD, T>
    create_mass_from_disjunctive_weight(std::size_t set, T v)
    {
        mass<FOD, T> m;
        m.at_emptyset() = v;
        m[set] = 1 - v;
    ...
    // mf12 = bft::return_fuse(rule_conjunctive(), mf1, mf2); // const // mf12
    // <- mf1 \opconj mf2
    ...
    {
        BOOST_ASSERT(i < FOD::powerset_size);
        return !detail::is_small(this->values()[i], detail::tolerance);
    }
    ...
    {
        return is_focal(
            bft_function<FOD, T>::template Idx<S0, S1, S2, S3>::value);
    }
    ...
    {
        return is_focal(
            bft_function<FOD, T>::template Idx<S0, S1, S2, S3, S4>::value);
    }
    ...
    {
        return is_focal(
            bft_function<FOD, T>::template Idx<S0, S1, S2, S3, S4, S5>::value);
    }
    ...
    bool is_focal() const
    {
        return is_focal(bft_function<FOD, T>::template Idx<S0, S1, S2, S3, S4,
                                                           S5, S6>::value);
    }
    ...
    bool is_focal() const
    {
        return is_focal(bft_function<FOD, T>::template Idx<
            S0, S1, S2, S3, S4, S5, S6, S7, S8>::value);
    }
    ...
    {
        return is_focal(bft_function<FOD, T>::template Idx<
            S0, S1, S2, S3, S4, S5, S6, S7, S8, S9>::value);
    }
    ...
    {
        return detail::is_small(at_omega(), detail::tolerance);
    }
    ...
    {
        return detail::is_small(1 - at_omega(), detail::tolerance);
    }

    /// Degenerate mass function has all mass at Emptyset (Conflict).
    bool is_degenerate() const
    {
        return detail::is_small(1 - at_emptyset(), detail::tolerance);
    }
    ...
    bool is_simple() const
    {
        ...
        bool has_two_focal_sets =
            (2 == focal_set_count); // unnecessary with the inner if-clause
        bool is_omega_a_focal_set = is_focal(FOD::powerset_size - 1);
        return has_one_focal_set ||
               (has_two_focal_sets && is_omega_a_focal_set);
    }
    ...
    /// focal sets.
    ///
    /// Consistent evidence means that there is at least one set that is common
    /// to all
    /// focal sets.
    ...
    /// Disjoint evidence implies that any two subsets have no elements in
    /// common with any
    /// other subset.
    ...
    bool is_partitioned() const
    {
        ...
        for (std::size_t A = focals_union + 1; A < FOD::powerset_size; ++A) {
            if (is_focal(A)) {
                // focals_intersection = set_intersection(A,
                // focals_intersection);
                // focals_intersections_union = set_union(focals_intersection,
                // focals_intersections_union);
    ...
    bool has_internal_conflict() const
    {
        ...
        for (std::size_t A = focals_intersection + 1; A < FOD::powerset_size;
             ++A) {
            if (is_focal(A)) {
                focals_intersection = set_intersection(A, focals_intersection);
            }
        }
        // has internal conflict if intersection of all focal elements is an
        // emptyset
