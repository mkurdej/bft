...
struct rule_pcr5 : public rule_base
{
    ...
    operator()(const mass<FOD, T>& m1, const mass<FOD, T>& m2) const
    {
        ...
        for (std::size_t B = 0; B < FOD::powerset_size; ++B) {
            ...
            for (std::size_t C = 0; C < FOD::powerset_size; ++C) {
                ...
                if (is_emptyset(A)) {
                    // redistribute conflicting mass to m[B] and m[C]
                    // proportionally to m1[B], m2[C]
                    T proportion_denominator = m1[B] + m2[C];
                    BOOST_ASSERT_MSG(
                        (proportion_denominator > detail::tolerance),
                        "denominator is close to zero!");
                    T conflict_mass = m1[B] * m2[C];
                    m_result[B] +=
                        conflict_mass * (m1[B] / proportion_denominator);
                    m_result[C] +=
                        conflict_mass * (m2[C] / proportion_denominator);
                } else {
                    // conjunctive
