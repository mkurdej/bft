...
template <class S0 = detail::EmptyType, class S1 = detail::EmptyType,
          class S2 = detail::EmptyType, class S3 = detail::EmptyType,
          class S4 = detail::EmptyType, class S5 = detail::EmptyType,
          class S6 = detail::EmptyType, class S7 = detail::EmptyType,
          class S8 = detail::EmptyType, class S9 = detail::EmptyType>
class fod
{
    ...
    /// can contain EmptyType
    typedef typename mpl::fold<template_types, mpl::set0<>,
                               mpl::insert<mpl::_1, mpl::_2> >::type
        types_with_empty_type;

public:
    typedef fod<S0, S1, S2, S3, S4, S5, S6, S7, S8, S9> this_type;

    /// set of classes, constructed from a list of potentially non-unique types
    typedef typename mpl::erase_key<types_with_empty_type,
                                    detail::EmptyType>::type type;
    ...
    {
        enum {
            value = is_same<T, S0>::value * 0 + is_same<T, S1>::value * 1 +
                    is_same<T, S2>::value * 2 + is_same<T, S3>::value * 3 +
                    is_same<T, S4>::value * 4 + is_same<T, S5>::value * 5 +
                    is_same<T, S6>::value * 6 + is_same<T, S7>::value * 7 +
                    is_same<T, S8>::value * 8 + is_same<T, S9>::value * 9
        };
